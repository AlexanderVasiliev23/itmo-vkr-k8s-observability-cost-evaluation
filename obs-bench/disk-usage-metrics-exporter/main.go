...
func statfsBytes(ctx context.Context, logger *slog.Logger, path string) (used, available, capacity uint64, err error) {
...
func collectOnce(ctx context.Context, logger *slog.Logger, mountPath string) (used, available, capacity uint64, err error) {
	...
	_, available, capacity, err = statfsBytes(ctx, logger, mountPath)
	if err != nil {
